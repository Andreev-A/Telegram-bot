    ...
    buttons = [
        [
            types.InlineKeyboardButton(
                text="Перенести стиль", callback_data="button_transfer_style"
            )
        ],
        [
            types.InlineKeyboardButton(
                text="\U0001F40E Превратить лошадь в зебру \U0001F993",
                callback_data="button_into_zebra",
            )
        ],
        [
            types.InlineKeyboardButton(
                text="\U0001F993 Превратить зебру в лошадь \U0001F40E",
...
def select_style() -> types.InlineKeyboardMarkup:
    buttons = [
        ...
        [
            types.InlineKeyboardButton(
                text="Отправить фотографию для стиля", callback_data="button_style_photo"
            )
        ],
...
def repeat_transform() -> types.InlineKeyboardMarkup:
    buttons = [
        [
            types.InlineKeyboardButton(
                text="Повторить с выбранным стилем", callback_data="button_style_repeat"
            )
        ],
        [types.InlineKeyboardButton(text="Возврат в меню", callback_data="button_menu")],
    ]
...
menu = (
    f"{hide_link(IMAGE_PATH + 'main_image.png')}"
...
async def send_menu(message: types.Message) -> None:
    await message.answer(f"{menu}", reply_markup=select_transform())
...
async def cycle_gan(
    message: types.Message, image: t.Optional[t.BinaryIO], type_algorithm: str
) -> None:
    wts_path = (
        "models_wts/horse2zebra.pth"
...
async def style_transfer(
    ...
    image_style: t.Optional[t.Union[str, t.BinaryIO]],
    image_basic: t.Optional[t.BinaryIO],
) -> None:
...
async def download_photo(message: types.Message) -> None:
    ...
    if user_algorithm in ("horse_into_zebra", "zebra_into_horse"):
        ...
        threading.Thread(
            target=lambda mess, img, type_algorithm: asyncio.run(
                cycle_gan(mess, img, type_algorithm)
            ),
            args=(message, photo, user_algorithm),
        ).start()
        ...
        if user_algorithm == "style_photo":
            ...
            await message.answer(
                "<b>Отправь фотографию, на которую надо перенести стиль</b>",
            ...
            threading.Thread(
                target=lambda mess, style_img, basic_img: asyncio.run(
                    style_transfer(mess, style_img, basic_img)
                ),
                args=(message, db_users[message.from_user.id]["style"], photo),
            ).start()
    else:
        await message.answer(
            "ВЫБЕРИ что надо сделать,\n" "а потом отправь мне фотографии  \U0001F447",
            reply_markup=select_transform(),
        )
